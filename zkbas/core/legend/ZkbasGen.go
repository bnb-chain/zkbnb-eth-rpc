...
func (_Zkbas *ZkbasCaller) Keccak256Hash(opts *bind.CallOpts, input []byte) ([32]byte, error) {
	var out []interface{}
	err := _Zkbas.contract.Call(opts, &out, "keccak256Hash", input)
...
//
// Solidity: function keccak256Hash(bytes input) view returns(bytes32 result)
func (_Zkbas *ZkbasSession) Keccak256Hash(input []byte) ([32]byte, error) {
	return _Zkbas.Contract.Keccak256Hash(&_Zkbas.CallOpts, input)
}

// Keccak256Hash is a free data retrieval call binding the contract method 0xeb90f459.
//
// Solidity: function keccak256Hash(bytes input) view returns(bytes32 result)
func (_Zkbas *ZkbasCallerSession) Keccak256Hash(input []byte) ([32]byte, error) {
	return _Zkbas.Contract.Keccak256Hash(&_Zkbas.CallOpts, input)
}
...
// ZkbasDebugInfoIterator is returned from FilterDebugInfo and is used to iterate over the raw logs and unpacked data for DebugInfo events raised by the Zkbas contract.
type ZkbasDebugInfoIterator struct {
	Event *ZkbasDebugInfo // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *ZkbasDebugInfoIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *ZkbasDebugInfoIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *ZkbasDebugInfoIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// ZkbasDebugInfo represents a DebugInfo event raised by the Zkbas contract.
type ZkbasDebugInfo struct {
	A   *big.Int
	B   *big.Int
	C   *big.Int
	Raw types.Log // Blockchain specific contextual infos
...
// FilterDebugInfo is a free log retrieval operation binding the contract event 0x4084de264fe5b4016bbd3867a02b3c42572dbf43e2b8968ad49fb57c43cd8d81.
//
// Solidity: event DebugInfo(uint256 a, uint256 b, uint256 c)
func (_Zkbas *ZkbasFilterer) FilterDebugInfo(opts *bind.FilterOpts) (*ZkbasDebugInfoIterator, error) {

	logs, sub, err := _Zkbas.contract.FilterLogs(opts, "DebugInfo")
	if err != nil {
		return nil, err
	}
	return &ZkbasDebugInfoIterator{contract: _Zkbas.contract, event: "DebugInfo", logs: logs, sub: sub}, nil
}

// WatchDebugInfo is a free log subscription operation binding the contract event 0x4084de264fe5b4016bbd3867a02b3c42572dbf43e2b8968ad49fb57c43cd8d81.
//
// Solidity: event DebugInfo(uint256 a, uint256 b, uint256 c)
func (_Zkbas *ZkbasFilterer) WatchDebugInfo(opts *bind.WatchOpts, sink chan<- *ZkbasDebugInfo) (event.Subscription, error) {

	logs, sub, err := _Zkbas.contract.WatchLogs(opts, "DebugInfo")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(ZkbasDebugInfo)
				if err := _Zkbas.contract.UnpackLog(event, "DebugInfo", log); err != nil {
					return err
				}
				event.Raw = log
...
// ParseDebugInfo is a log parse operation binding the contract event 0x4084de264fe5b4016bbd3867a02b3c42572dbf43e2b8968ad49fb57c43cd8d81.
//
// Solidity: event DebugInfo(uint256 a, uint256 b, uint256 c)
func (_Zkbas *ZkbasFilterer) ParseDebugInfo(log types.Log) (*ZkbasDebugInfo, error) {
	event := new(ZkbasDebugInfo)
	if err := _Zkbas.contract.UnpackLog(event, "DebugInfo", log); err != nil {
		return nil, err
	}
